...
function showError(message) {
  ...
  errorEl.innerHTML = `<h3>错误</h3><p>${message}</p>`;
  errorEl.style.display = 'block';
}

// 更新加载进度
function updateLoadProgress() {
  appState.loadedResources++;
  appState.loadProgress = (appState.loadedResources / appState.totalResources) * 100;
  document.getElementById('progress').style.width = `${appState.loadProgress}%`;
  document.getElementById('loading-text').textContent = `加载中... ${Math.round(appState.loadProgress)}%`;
  
  // 加载完成隐藏加载界面
  if (appState.loadProgress >= 100) {
    setTimeout(() => {
...
function initPrologueScene() {
  ...
  textureLoader.load(appState.prologueBackgroundImage, 
    ...
    (error) => {
      ...
      updateLoadProgress();
      showError(`无法加载背景图片: ${appState.prologueBackgroundImage}`);
    }
  );
...
function initStartScene() {
  ...
  pngLoader.load(appState.catImageSequence[0], 
    ...
    (error) => {
      ...
      updateLoadProgress();
      showError(`无法加载图片: ${appState.catImageSequence[0]}`);
    }
  );
  ...
  function loadModel(modelInfo) {
    ...
    loader.load(
      ...
      (error) => {
        ...
        updateLoadProgress();
        showError(`无法加载模型: ${modelInfo.path}`);
      }
    );
  }
...
function initBackgroundMusic() {
  try {
    ...
    console.log('背景音乐初始化完成，等待用户交互后播放');
    updateLoadProgress();
  } catch (error) {
...
function validateCursorImages() {
  const validateImage = (src, name) => {
    ...
    img.src = src;
    img.onload = () => {
      console.log(`${name}图片加载成功`);
    };
    img.onerror = () => {
      console.error(`${name}图片加载失败，请检查路径: ${src}`);
      showError(`图片加载失败: ${src}`);
    };
  };
    ...
    if (progress < 1) {
      ...
      textureLoader.load(appState.catImageSequence[imageIndex], 
        ...
        (error) => {
          ...
          showError(`无法加载图片: ${appState.catImageSequence[imageIndex]}`);
        }
      );
    }
  }
